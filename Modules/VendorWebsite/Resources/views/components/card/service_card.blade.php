...
<div class="service-card bg-gray-800 p-4 rounded mb-3 cursor-pointer" id="service-card-{{ $service->id ?? rand() }}"
    data-service-id="{{ $service->id ?? '' }}">
    @if ($service->is_platinum_member ?? false)
        <div class="service-card-top-content">
            <span
    ...
    <div class="service-card-inner">
        ...
        <div class="service-card-content">
            <div class="d-flex align-items-baseline flex-sm-row flex-column column-gap-2 row-gap-3">
                <div class="d-flex align-items-sm-baseline gap-3 flex-wrap flex-sm-row flex-grow-1">
                    @if ($service->category_name ?? ($service->category->name ?? false))
                        <span
                            class="font-size-10 bg-success-subtle border border-success fw-bold rounded-pill px-2 py-1 flex-shrink-0 lh-1">{{ strtoupper($service->category_name ?? $service->category->name) }}</span>
                    @endif
                ...
                <div class="d-flex align-items-baseline justify-content-end row-gap-2 column-gap-4 flex-grow-1">
                    <div class="d-flex align-items-center gap-2">
                        @if (session('selected_branch_id'))
                            <span
                                class="fw-medium text-primary font-size-21-3 lh-1">{{ Currency::vendorCurrencyFormate($service->branchServices->first()->service_price ?? $service->default_price) }}</span>
                        @else
                            <span
                                class="fw-medium text-primary font-size-21-3 lh-1">{{ Currency::vendorCurrencyFormate($service->default_price) }}</span>
                        @endif

                    </div>
                    <div class="form-check">
                        <input class="form-check-input form-check-secondary service-checkbox" type="checkbox"
                ...
                @if (session('selected_branch_id'))
                    ...
                    <li>
                        <span class="d-flex align-items-center gap-2">
                            ...
                            <!-- <span class="font-size-14">{{ $service->duration_min }}
                                {{ __('vendorwebsite.min') }}</span> -->
                            <span
                                class="font-size-14">{{ $service->branchServices->first()->duration_min ?? ($service->duration_min ?? 0) }}
                                {{ __('vendorwebsite.min') }}</span>
                        </span>
                    </li>
                @else
                    @if ($service->duration_min ?? false)
                        <li>
                            <span class="d-flex align-items-center gap-2">
                                <i class="ph ph-clock"></i>
                                <span class="font-size-14">{{ $service->duration_min ?? 0 }}
                                    {{ __('vendorwebsite.min') }}</span>
                            </span>
                        </li>
                    @endif
                            ...
                            <div class="service-card-inner cursor-pointer"
                                ...
                                <div class="service-card-content">
                                    <div
                                        ...
                                        <div
                                            class="d-flex align-items-sm-baseline gap-3 flex-wrap flex-sm-row flex-grow-1">
                                        ...
                                        <div
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                @if (isset($addon->original_price) && $addon->original_price > ($addon->price ?? 0))
                                                    <span
                                                        class="fw-medium text-decoration-line-through font-size-14 lh-1">{{ \Currency::vendorCurrencyFormate($addon->original_price) }}</span>
                                                @endif
                                            </div>
                                            <div class="form-check">
