...
class WalletController extends Controller
{
    ...
    {
        $transaction = Wallet::findOrFail($transaction_id);
        $transaction->update(['status' => 'success']);
        $transaction->user->wallet()->increment('amount', $transaction->amount);

        return redirect()->route('wallet.view')->with('success', 'Top-up successful!');
    }

    public function paymentCancel($transaction_id)
    {
        Wallet::findOrFail($transaction_id)->update(['status' => 'cancelled']);
        return redirect()->route('wallet.view')->with('error', 'Payment cancelled.');
    }

    // Unified redirect handler for Razorpay, Flutterwave, Paystack
    public function gatewayRedirect($method, $transaction_id)
    {
        $transaction = Wallet::findOrFail($transaction_id);

        // Load specific view for each gateway or integrate APIs
        return view("payments.$method", compact('transaction'));
    }
