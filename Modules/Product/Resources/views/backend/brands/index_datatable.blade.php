...
@section('content')
    ...
    @include('product::backend.brands.form_offcanvas')
...
@push('after-scripts')
    ...
    <script type="text/javascript" defer>
        ...
        // Function to handle edit brand action
        function editBrandAction(brandId) {
            console.log('editBrandAction called with ID:', brandId);
            
            fetch(`{{ route('backend.brands.edit', ':id') }}`.replace(':id', brandId))
                .then(response => {
                ...
                .then(data => {
                    ...
                    if (data.status) {
                        console.log('Calling window.editBrand with data:', data.data);
                        window.editBrand(data.data);
                    } else {
                        console.error('Error in response:', data.message);
                        window.errorSnackbar(data.message || 'Error loading brand data');
                    }
                })
                .catch(error => {
                    console.error('Fetch error:', error);
                    window.errorSnackbar('An error occurred while loading brand data');
                });
        }
