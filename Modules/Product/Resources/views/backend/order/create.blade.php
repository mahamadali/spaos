    ...
    <div class="card shadow rounded-2 mb-4 p-4">
        ...
        <div class="row gy-4">
            <div class="col-lg-4 col-md-6">
                <label for="branch-id" class="form-label">{{ __('messages.branch') }}</label>
                <select id="branch-id" name="branch_id" class="form-select select2" data-placeholder="{{ __('messages.select_branch') }}" required>
            ...
            <div class="col-lg-4 col-md-6">
                <label for="user-id" class="form-label">{{ __('messages.customer') }}</label>
                <select id="user-id" name="user_id" class="form-select select2" data-placeholder="{{ __('messages.select_customer') }}" required>
            ...
            <div class="col-md-4">
                ...
                <div style="width: 100%; display: inline-block;">
                    <select name="delivery_status" id="delivery-status" class="select2 form-control" data-filter="select"
                        style="width: 375px" required>
                        <option value="">{{ __('messages.select_delivery_status') }}</option>
                ...
                <div class="row align-items-center">
                    <div class="col-8">
                        ...
                        <div class="input-group flex-nowrap">
                            <select id="address-id" name="address_id" class="form-select" aria-label="User addresses" required>
                    ...
                    <form id="add-address-form" action="{{ route('backend.orders.store_address') }}" method="POST">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select select2" id="state" name="state" disabled>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select select2" id="city" name="city" disabled>
...
let is_discount_applicable = false;

if (discount_start_date && discount_end_date) {
  const current_date = new Date();

  const today = current_date.toLocaleDateString('en-CA'); // YYYY-MM-DD
  const startDate = new Date(discount_start_date * 1000).toLocaleDateString('en-CA');
  const endDate = new Date(discount_end_date * 1000).toLocaleDateString('en-CA');

  is_discount_applicable = today >= startDate && today <= endDate;

  console.log({ today, startDate, endDate, is_discount_applicable });
}
                let discountValue = 0;
        ...
        $(document).ready(function() {
            ...
            $('#country').change(function() {
                ...
                if (countryId) {
                    ...
                    // Revalidate state field when country changes
                    $('#state').valid();
                    $.ajax({
            ...
            $('#state').change(function() {
                ...
                // Revalidate state and city when state changes
                $('#state').valid();
                if (stateId) {
            ...
            // Initialize jQuery Validate for Address Form
            if (window.$ && $.fn && $.fn.validate) {
                // Custom validation method for state (required when country is selected, or show error from start if country is selected)
                $.validator.addMethod('validateState', function(value, element) {
                    ...
                    // If no country, state is not required
                    return true;
                }, '{{ __("messages.states") }} is a required field');

                // Custom validation method for city (required when state is selected, or show error from start if state is selected)
                $.validator.addMethod('validateCity', function(value, element) {
                    ...
                    // If no state, city is not required
                    return true;
                }, '{{ __("messages.cities") }} is a required field');
                ...
                $('#add-address-form').validate({
                    ...
                    errorPlacement: function(error, element) {
                        ...
                        if (errorDiv.length === 0) {
                            errorDiv = $('<div>').attr('id', errorId).addClass('d-block text-danger mt-1');
                            if ($element.hasClass('select2-hidden-accessible')) {
                                $element.next('.select2-container').after(errorDiv);
                            } else {
                                $element.after(errorDiv);
                            }
                        }
                        errorDiv.text(error.text()).addClass('d-block text-danger').removeClass('text-muted');
                    },
                    highlight: function(element) {
                        // Don't add red border, just keep error message
                        const $element = $(element);
                    ...
                    invalidHandler: function(event, validator) {
                        // Focus first invalid field (but don't open Select2 dropdown)
                    ...
                    messages: {
                        first_name: {
                            required: '{{ __("messages.first_name") }} is a required field'
                        },
                        last_name: {
                            required: '{{ __("messages.last_name") }} is a required field'
                        },
                        country: {
                            required: '{{ __("messages.countries") }} is a required field'
                        },
                        state: {
                            required: '{{ __("messages.states") }} is a required field'
                        },
                        city: {
                            required: '{{ __("messages.cities") }} is a required field'
                        },
                        postal_code: {
                            required: '{{ __("messages.postal_code") }} is a required field'
                        },
                        address_line_1: {
                            required: '{{ __("messages.address") }} is a required field'
                        }
                    },
                    submitHandler: function(form) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                setTimeout(() => {
                                    if (currentVariantId) {
                                        $('#variant-id').val(currentVariantId).trigger('change');
                                    }
                                }, 300);

                                // Clear only address-related fields and reset validation
                                $form.find('#country, #state, #city').val('').trigger('change');
                                $('#firstName, #lastName, #pinCode, #address').val('');
                ...
                $('#country, #state, #city').on('change.select2 change', function() {
                    const $field = $(this);
                    $field.valid();
                    // If country changes, validate state
                    if ($field.attr('id') === 'country') {
                        $('#state').valid();
                    }
                    // If state changes, validate city
                    if ($field.attr('id') === 'state') {
                        $('#city').valid();
                    }
                });

                // Reset validation when modal is closed
                $('#modalAddAddress').on('hidden.bs.modal', function() {
                    $('#add-address-form').validate().resetForm();
                    $('#add-address-form').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
                    $('#add-address-form').find('[id$="-error"]').remove();
                });

                // Validate state and city when modal is opened (to show errors if country/state is selected)
                $('#modalAddAddress').on('shown.bs.modal', function() {
                    // Revalidate state and city to show errors if needed
                    $('#state, #city').valid();
                });
            }
            ...
            function updateCartDisplay() {
                ...
                const validator = $('#cart-items-validator');
                if (validator.length) {
                    if (cart.length > 0) {
                        validator.val('has_items');
                    } else {
                        validator.val('');
                    }
                    validator.valid();
...
if (discount_start_date && discount_end_date) {
  ...
  console.log({ today, startDate, endDate, is_discount_applicable });
            ...
            // Initialize jQuery Validate (must be inside document ready)
            $(document).ready(function() {
                ...
                messages: {
                    ...
                    delivery_status: {
                        required: 'Please select a delivery status'
                    },
                    cart_items: {
                        required: 'Please add items to the cart'
                    }
                },
                submitHandler: function(form) {
                    placeOrder();
                ...
                $.validator.addMethod('cart_items', function(value, element) {
                    return cart.length > 0;
                }, 'Please add items to the cart');

                // Add a hidden field for cart validation
                if (!$('#cart-items-validator').length) {
                    $('#order-create-form').append('<input type="hidden" id="cart-items-validator" name="cart_items" value="">');
                }
            });
            ...
            $(document).on('click', '#place-order-btn', function(e) {
                ...
                if ($('#order-create-form').valid()) {
                    ...
                    const firstError = $('#order-create-form').find('.is-invalid').first();
            ...
            function placeOrder() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        toastr.success('Order save successfully!');
                        cart = [];
                        updateCartDisplay();
                        $('#branch-id, #user-id, #address-id, #product-id, #variant-id').val('')
                            .trigger('change');
                        $('#order-create-form').validate().resetForm();
