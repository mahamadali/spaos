...
class OrdersController extends Controller
{
    ...
    public function store(OrderRequest $request)
    {
        ...
        //$carts = Cart::where('user_id', $userId)->where('location_id', $location_id)->get(); // apply when set location_id

        // Get cart items only for the current vendor
        ...
        if (count($carts) > 0) {
            ...
            foreach ($carts as $cart) {
                ...
                if ($cart->qty > $productVariationStock) {
                    if ($cart->product_variation) {
                        ...
                        $message = $cart->product_variation->product->name . ' is out of stock' ?? 'Product is out of stock';
                    } else {
            ...
            foreach ($carts as $cart) {
                ...
                // Only delete cart items from the current vendor
                if ($cart->product && $cart->product->created_by == session('current_vendor_id')) {
                    $cart->delete();
                }
            }
            ...
            try {
                ...
                if(session()->has('current_vendor_id')){
                    $notification_data['vendor_id'] = session()->get('current_vendor_id');
                }
    ...
    public function cancelOrder(Request $request, $orderId)
    {
        ...
        $order->save();
        // Optionally, add an order update record
        OrderUpdate::create([
            'order_id' => $order->id,
            'user_id' => $userId,
