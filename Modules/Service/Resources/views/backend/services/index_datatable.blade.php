...
@push('after-styles')
    ...
    <link rel="stylesheet" href="{{ asset('css/service-form-offcanvas.css') }}?v={{ time() }}">
@endpush
...
@push('after-scripts')
    ...
    <script>
        window.ServiceRoutes = {
            editForm: @json(route('backend.services.get_edit_form', ['id' => '__ID__'])),
            getSubcategories: @json(route('backend.services.get_subcategories')),
        };
    </script>
    <script type="text/javascript" defer>
        ...
        $(document).off('click', '[data-assign-event="employee_assign"]').on('click', '[data-assign-event="employee_assign"]', function() {
        var serviceId = $(this).data('assign-module');
        var urlTemplate = @json(route('backend.services.assign_employee_offcanvas', ['id' => '__ID__']));
        var requestUrl = urlTemplate.replace('__ID__', serviceId);
        $.get(requestUrl, function(html) {
        $('#employee-offcanvas-container').html(html);
        var offcanvas = new bootstrap.Offcanvas(document.getElementById('service-employee-assign-form'));
        offcanvas.show();
        });
    });
    </script>
