...
@section('content')
    ...
    @include('earning::backend.earnings.form_offcanvas', [
        'createTitle' => __('messages.create') . ' ' . __('messages.new') . ' ' . __($module_title),
        'editTitle'   => __('messages.create') . ' ' . __('messages.create') . ' ' . __('messages.staff_payout'),
    ])
@endsection
    ...
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', (event) => {
            window.renderedDataTable = $('#datatable').DataTable({
                ...
                columnDefs: [
                    { targets: [2, 3, 4, 5], className: "text-end" } // right align only amounts
                ]
            });
        })
