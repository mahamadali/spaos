...
trait PaymentTrait
{
    public function getpayment_method($data, $booking_id)
    {
        ...
        if (!empty($booking_transaction_data)) {
           ...
           $booking_service = BookingService::where('booking_id', $booking_id)->first();
            ...
            if(isset($booking_transaction) && $booking_transaction['payment_status'] == 1){



                CommissionEarning::where('commissionable_type', 'Modules\Booking\Models\Booking')->where('commissionable_id', $booking_id)->update(['commission_status' => 'unpaid']);
        ...
        if ($tax != '') {
            foreach ($tax as $tax_value) {
                if ($tax_value['type'] == 'percent') {
                    ...
                    $tax_amount = $tax_amount + ($tax_value['tax_amount'] ?? 0);
                }
            }
        }
    ...
    public function couponDiscount($total_amount, $coupon_code, $booking_id)
    {
        ...
        if ($coupon) {
            $couponDiscountamount = $coupon->discount_type == 'percent' ? $total_service_amount * ($coupon->discount_percentage / 100) : $coupon->discount_amount;
        }
    ...
    public function getcashpayments($data, $booking_transaction_id)
    {
        ...
        CommissionEarning::where('commissionable_type', 'Modules\Booking\Models\Booking')->where('commissionable_id', $data['booking_id'])->update(['commission_status' => 'unpaid']);
    ...
    public function commissionData($data)
    {
        ...
        if ($booking_transaction && $booking_transaction->payment_status == 1) {
