...
trait BookingTrait
{
    public function updateBookingService($data, $booking_id)
    {
        ...
        // Get booking to fetch branch_id for branch-specific pricing
        $booking = \Modules\Booking\Models\Booking::find($booking_id);
        $branchId = $booking ? $booking->branch_id : null;
        
        foreach ($serviceData as $key => $value) {
            // Fetch latest service price from database instead of using request data
            $service = \Modules\Service\Models\Service::find($value['service_id']);
            $latestServicePrice = 0;
            
            if ($service) {
                // If branch_id exists, try to get branch-specific price
                if ($branchId) {
                    $branchService = ServiceBranches::where('service_id', $value['service_id'])
                        ->where('branch_id', $branchId)
                        ->first();
                    if ($branchService) {
                        $latestServicePrice = $branchService->service_price ?? 0;
                    }
                }
                
                // Fallback to default_price if branch price not found or branch_id not available
                if ($latestServicePrice == 0) {
                    $latestServicePrice = $service->default_price ?? 0;
                }
            }
            
            // Use latest price from database, fallback to request price if service not found
            $servicePrice = $latestServicePrice > 0 ? $latestServicePrice : ($value['service_price'] ?? 0);
    ...
    public function getSlots($date, $day, $branch_id, $serviceDuration=0, $employee_id = null)
    {
        ...
        $is_holiday = Holiday::where('date', $date)->where('branch_id', $branch_id)->exists();
    ...
    protected function sendNotificationOnBookingUpdate($type, $notify_message, $booking, $notify = true)
    {
        ...
        $data['booking'] = [
            ...
            'vendor_id' => optional($booking->branch)->vendor_id ?? optional($booking->branch)->created_by,
