        ...
        if (!empty($request->packages) && (!$request->has('is_reclaim') || !$request->is_reclaim)) {
            ...
            foreach ($request->packages as $key => $value) {
                ...
                if ($existingPackage) {
                    ...
                    return response()->json(['message' => __('messages.already_purchased_package'), 'status' => false], 200);
                }
            }
        }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->has('status') && $request->status == 'cancelled') {
            ...
            if (!in_array($booking->status, ['check_in', 'checkout', 'completed'])) {
                ...
                $message = __('messages.cannot_cancel_booking_with_status');
                return response()->json(['message' => "$message {$booking->status}"], 422);
            }
        } else {
