...
use Modules\Commission\Models\CommssionEarning;
use Auth;
...
class BookingsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('booking::backend.bookings.index', compact('module_action', 'statusList', 'date', 'module_title','branch_id'));
    }
   ...
   public function index_list(Request $request)
{
    ...
    foreach ($data as $key => $value) {
        ...
        // use DB datetime as-is (no timezone conversion)
        $startTime = $value->start_date_time instanceof \Carbon\Carbon
            ? $value->start_date_time
            : \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $value->start_date_time);
    ...
    foreach ($package as $key => $value) {
        ...
        $startTime = $value->booking->start_date_time instanceof \Carbon\Carbon
            ? $value->booking->start_date_time
            : \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $value->booking->start_date_time);
        ...
        $resource[] = [
            ...
            'titleHTML' => '<div class="d-flex gap-3 justify-content-center align-items-center py-3">
                                <img src="' . $employee->profile_image . '"
                                     ...
                                     alt="employee" />' . $employee->full_name . '</div>',
        ];
    }
    ...
    public function store(BookingRequest $request)
    {
        ...
        // Snapshot current taxes at BOOKING time so future tax changes don't affect this booking
        try {
            $existingTx = BookingTransaction::where('booking_id', $booking->id)->first();
            if (!$existingTx) {
                $taxes = Tax::where('status', 1)
                    ...
                    ->map(function ($tax) {
                        ...
                        // For fixed tax, amount should be the tax value itself
                        // For percent tax, amount will be calculated later based on subtotal
                        $amount = $tax->type === 'fixed' ? $taxValue : 0;
                        
                        \Log::info('🔵 Booking Store - Tax preparation', [
                        ...
                        return [
                            ...
                            'percent'    => $taxValue,
                            'tax_amount' => $amount,
                            'amount'     => $amount, // Ensure amount is set correctly for fixed taxes
                        ];
                    })
                    ->toArray();
                
                \Log::info('🔵 Booking Store - Taxes before saving', [
                    'booking_id' => $booking->id,
                    'taxes' => $taxes,
                ]);
                ...
                \Log::info('🔵 Booking Store - BookingTransaction created', [
                    'booking_id' => $booking->id,
                ]);
            }
        } catch (\Throwable $e) {
            \Log::warning('Failed to snapshot taxes for booking '.$booking->id.': '.$e->getMessage());
        }
    ...
    public function payment_create(Request $request)
    {
        ...
        // Prefer stored snapshot regardless of payment status
        $storedTx = BookingTransaction::where('booking_id', $booking_id)->first();
        $taxes = $storedTx->tax_percentage ?? null;
        
        \Log::info('🔵 Payment Create - Tax retrieval', [
            'booking_id' => $booking_id,
            'has_stored_tx' => $storedTx ? true : false,
            'taxes_raw' => $taxes,
            'taxes_type' => gettype($taxes),
            'is_array' => is_array($taxes),
            'taxes_count' => is_array($taxes) ? count($taxes) : 0,
        ]);
        ...
        if (!$taxes || !is_array($taxes) || empty($taxes)) {
            \Log::info('🔵 Payment Create - No stored taxes, fetching from Tax model');
            $taxes = Tax::where('status', 1)
                ->where(function ($query) {
                    $query->whereNull('module_type')
                          ->orWhere('module_type', 'services');
                ...
                ->map(function ($tax) use ($total_service_amount) {
                    ...
                    \Log::info('🔵 Payment Create - Tax from model', [
                        'name' => $tax->title,
                        'type' => $tax->type,
                        'value' => $tax->value,
                        'calculated_amount' => $amount,
            ...
            \Log::info('🔵 Payment Create - Normalizing stored taxes', [
                'taxes_before' => $taxes,
            ]);
            
            // Normalize stored tax structure to ensure it has the correct fields
            $taxes = array_map(function ($tax) use ($total_service_amount) {
                $taxType = $tax['type'] ?? 'percent';
                $taxName = $tax['name'] ?? $tax['title'] ?? 'Tax';
                
                \Log::info('🔵 Payment Create - Processing tax item', [
                    'original_tax' => $tax,
                    'tax_type' => $taxType,
                    'tax_name' => $taxName,
                    'has_amount' => isset($tax['amount']),
                    'has_tax_amount' => isset($tax['tax_amount']),
                    'has_value' => isset($tax['value']),
                    'amount_value' => $tax['amount'] ?? null,
                    'tax_amount_value' => $tax['tax_amount'] ?? null,
                    'value_value' => $tax['value'] ?? null,
                ]);
                
                if ($taxType === 'fixed') {
                    // For fixed tax, ensure amount field is set correctly
                    $amount = (float) ($tax['amount'] ?? $tax['tax_amount'] ?? $tax['value'] ?? 0);
                    
                    \Log::info('🔵 Payment Create - Fixed tax normalization', [
                        'tax_name' => $taxName,
                        'extracted_amount' => $amount,
                        'source' => isset($tax['amount']) ? 'amount' : (isset($tax['tax_amount']) ? 'tax_amount' : (isset($tax['value']) ? 'value' : 'none')),
                    ]);
                    ...
                    // For percent tax, calculate amount based on current subtotal
                    $percent = (float) ($tax['percent'] ?? $tax['value'] ?? 0);
                    $amount = ($total_service_amount * $percent) / 100;
                    
                    \Log::info('🔵 Payment Create - Percent tax normalization', [
                        'tax_name' => $taxName,
                        'percent' => $percent,
                        'calculated_amount' => $amount,
                        'subtotal' => $total_service_amount,
                    ]);
            ...
            \Log::info('🔵 Payment Create - Taxes after normalization', [
                'taxes_after' => $taxes,
            ]);
        }
        ...
        \Log::info('🔵 Payment Create - Final response data', [
            'booking_id' => $booking_id,
    ...
    public function booking_payment(Request $request, Booking $booking_id)
    {
        ...
        $responseData['updated_at'] = $booking_details->updated_at ?? null;
    ...
    public function payment_success($id)
    {
        ...
        if ($session_object['payment_intent'] !== '' && $session_object['payment_status'] == 'paid') {
            ...
            Booking::where('id', $booking_id)->update(['status' => 'completed']);
             CommssionEarning::where('commissionable_type', 'Modules\Booking\Models\Booking')->where('commissionable_id', $booking_id)->update(['commission_status' => 'unpaid']);
    ...
    public function downloadInvoice(Request $request)
    {
        ...
        $view = view("mail.invoice-templates.template1", ['data' => $data['booking'], 'logo' => $logo])->render();
