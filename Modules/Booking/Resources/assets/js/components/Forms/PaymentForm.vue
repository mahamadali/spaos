  ...
  <div class="offcanvas-body">
    ...
    <div class="form-group d-flex align-items-center justify-content-between" v-for="(tax, index) in taxes" :key="index">
      <template v-if="tax.type == 'percent'">
        <label for="">{{ tax.name }}: {{ tax.percent + '%' }}</label>
        <strong>+  {{ formatCurrencyVue(calculatePercentAmount(tax.percent) )}}</strong>
      </template>
      <template v-else>
        <label for="">{{ tax.name }}: </label>
        <strong>+ {{ formatCurrencyVue(tax.amount) }}</strong>
      </template>
    </div>
