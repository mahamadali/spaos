    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ...
            ->addColumn('feature_image', function ($row) {
                $imageUrl = $row->getFirstMediaUrl('feature_image');
                if ($imageUrl) {
                    return '<img src="'.$imageUrl.'" alt="Banner Image" class="img-thumbnail" style="width: 60px; height: 40px; object-fit: cover;">';
                }
                return '<span class="text-muted">No Image</span>';
            })
            ->editColumn('name', function ($row) {
                return view('slider::backend.sliders.name_column', compact('row'));
            })
        ...
        return $datatable->rawColumns(array_merge(['action', 'status', 'check', 'feature_image', 'name', 'url']))
            ->toJson();
    }
