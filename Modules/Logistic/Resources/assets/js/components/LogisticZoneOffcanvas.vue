  ...
  <form @submit="formSubmit">
    <div class="offcanvas offcanvas-end" tabindex="-1" id="form-offcanvas" aria-labelledby="form-offcanvasLabel">
      ...
      <div class="offcanvas-body">
        <InputField class="col-md-12" type="text" :is-required="true" :label="$t('logistic_zone.lbl_name')"
          :placeholder="$t('logistic_zone.name')" v-model="name" :error-message="errors['name']"></InputField>
        <div class="col-md-12 form-group">
          <label class="form-label">{{ $t('logistic_zone.logistic') }} <span class="text-danger">*</span></label>
          <Multiselect id="logistic-list" v-model="logistic_id" :placeholder="$t('logistic_zone.logistic')"
            :value="logistic_id" v-bind="singleSelectOption" :options="logistics.options" class="form-group">
          </Multiselect>
        ...
        <div class="col-md-12 form-group">
          ...
          <Multiselect id="country-list" v-model="country_id" :placeholder="$t('logistic_zone.select_country')"
            :value="country_id" v-bind="singleSelectOption" :options="countries.options" @select="getState"
        ...
        <div class="col-md-12 form-group">
          ...
          <Multiselect id="state-list" v-model="state_id" :placeholder="$t('logistic_zone.select_state')"
            :value="state_id" v-bind="singleSelectOption" :options="states.options" @select="handleStateChange"
            class="form-group"></Multiselect>
        ...
        <div class="col-md-12 form-group">
          ...
          <Multiselect id="city-list" v-model="city_id" :placeholder="$t('logistic_zone.select_city')" :value="city_id"
            v-bind="multiselectOption" :options="cities.options" class="form-group"></Multiselect>
        ...
        <InputField class="col-md-12" type="number"
          :label="`${$t('logistic_zone.standard_delivery_charge')} ${currencySymbol}`" placeholder=""
          v-model="standard_delivery_charge" :error-message="errors['standard_delivery_charge']" />
        <InputField class="col-md-12" type="text" :label="$t('logistic_zone.standard_delivery_time')"
          :placeholder="$t('logistic_zone.delivery_time')" v-model="standard_delivery_time"
          :error-message="errors['standard_delivery_time']"></InputField>
      </div>
...
const validationSchema = yup.object({
  ...
  city_id: yup.array().min(1, 'City is a required field'),
  standard_delivery_charge: yup
    .number()
    .typeError('Standard delivery charge must be a number')
    .min(0, 'Standard delivery charge must be greater than or equal to 0'),
});
...
const handleStateChange = (value) => {
  // Clear cities immediately when state changes
  city_id.value = []
  ...
  if (currentId.value > 0) {
    getRequest({ url: EDIT_URL, id: currentId.value }).then((res) => {
      if (res.status) {
        ...
        useSelect({ url: STATE_URL, data: country_id.value }, { value: 'id', label: 'name' }).then((data) => {
          states.value = data
        })
