<div class="text-end">
    ...
    <a href="{{route('backend.users.changePassword', $data)}}" class="btn btn-secondary btn-icon btn-sm" data-bs-toggle="tooltip" data-bs-title="{{__('labels.backend.changePassword')}}"><i class="fas fa-key"></i></a>
    ...
    @if ($data->status == 2)
    <a href="{{route('backend.users.unblock', $data)}}" class="btn btn-secondary btn-icon btn-sm" data-method="PATCH" data-token="{{csrf_token()}}" data-bs-toggle="tooltip" data-bs-title="{{__('labels.backend.unblock')}}" data-confirm="{{ __('messages.are_you_sure?') }}"><i class="fas fa-check"></i></a>
    @endif
