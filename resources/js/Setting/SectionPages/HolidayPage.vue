...
const handleDateClick = (e) => {
  ...
  if (selectedDate.isBefore(today)) {
    // window.errorSnackbar('Past dates cannot be selected for holidays')
    return
  }
...
onMounted(() => {
  useSelect({ url: BRANCH_LIST }, { value: 'id', label: 'name' }).then((data) => {
    ...
    if (data.options.length === 1) {
      ...
      // Load existing holidays when there's only one branch
      fetchData()
    }
  })
})
...
const calendarOptions = ref({
  ...
  dayCellClassNames: (info) => {
    ...
    const today = moment().startOf('day')
    return date.isBefore(today) ? 'fc-past-date' : ''
  }
})
