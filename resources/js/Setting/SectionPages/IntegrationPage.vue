<template>
  <form @submit="formSubmit">
    ...
    <div class="form-group">
      <div class="d-flex justify-content-between align-items-center">
        <label class="form-label" for="category-is_demo_login">{{ $t('setting_integration_page.lbl_demo_login') }} </label>
        <div class="form-check form-switch">
          <input class="form-check-input" :true-value="1" :false-value="0" :value="is_demo_login" :checked="is_demo_login == 1 ? true : false" name="is_demo_login" id="category-is_demo_login" type="checkbox" v-model="is_demo_login" />
        </div>
      </div>
    </div>
    <div class="form-group">
...
const setFormData = (data) => {
  resetForm({
    values: {
      ...
      is_demo_login: data.is_demo_login || 0,
...
const { value: is_demo_login } = useField('is_demo_login')
