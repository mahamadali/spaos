          ...
          <div class="modal-body">
            <div class="row">
              <div class="col-12">
                ...
                <div class="form-group">
                  <label class="form-label" for="label">{{ $t('currency.lbl_currency_position') }} <span class="text-danger">*</span></label>
                  <span class="text-danger d-block mb-1" v-if="errors.currency_position">{{ errors.currency_position }}</span>
                  <select 
                    v-model="currency_position" 
                    class="form-select"
                    :key="`currency-position-${currentId}`"
                  >
                ...
                <div class="form-group">
                  <label class="form-label" for="label">{{ $t('currency.lbl_example') }} :-</label>
                  <div class="currency-example-display">
                    {{ formatCurrencyVue(labelValue, no_of_decimal, decimal_separator, thousand_separator, currency_position, currency_symbol) }}
                  </div>
                </div>
              </div>
            </div>
...
const formatCurrencyVue = window.formatCurrencyWithWrapping || window.formatCurrency
...
const setFormData = (data) => {
  resetForm({
    values: {
      ...
      no_of_decimal: data.no_of_decimal || 2,
...
const validationSchema = yup.object({
  currency_name: yup.string()
    ...
    .test('no-spaces-only', t('messages.cannot_contain_only_spaces'), (value) => value ? value.trim().length > 0 : true),
    currency_symbol: yup.string()
      .required(t('messages.currency_symbol_required'))
      .test('no-spaces-only', t('messages.cannot_contain_only_spaces'), (value) => value ? value.trim().length > 0 : true),
    currency_code: yup.string()
      .required(t('messages.currency_code_required'))
      .test('no-spaces-only', t('messages.cannot_contain_only_spaces'), (value) => value ? value.trim().length > 0 : true),
    currency_position: yup.string().required(t('messages.currency_position_required')),
    no_of_decimal: yup.string().required(t('messages.no_of_decimal_required')),
    thousand_separator: yup.string()
      .required(t('messages.thousand_separator_required'))
      .test('no-spaces-only', t('messages.cannot_contain_only_spaces'), (value) => value ? value.trim().length > 0 : true),
    decimal_separator: yup.string()
      .required(t('messages.decimal_separator_required'))
      .test('no-spaces-only', t('messages.cannot_contain_only_spaces'), (value) => value ? value.trim().length > 0 : true)
})
...
// Watch for currency_position changes to ensure proper reactivity
watch(currency_position, (newValue) => {
  console.log('Currency position changed to:', newValue)
}, { immediate: true })
