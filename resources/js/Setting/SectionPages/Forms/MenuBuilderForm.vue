          ...
          <div class="modal-body">
            <div class="row">
              <div class="col-12">
                ...
                <div class="form-group">
                  ...
                  <Multiselect v-model="title" :value="title" :placeholder="$t('menu_builder.lbl_title')"
                    ...
                    <span class="mt-3"><strong>{{ $t('messages.note') }} </strong>{{ $t('messages.type_and_press') }}</span>
                ...
                <InputField class="col-md-12" :is-required="false" :label="$t('menu_builder.lbl_icon')" placeholder=""
                  ...
                  <span class="mt-3"><strong>{{ $t('messages.note') }}: </strong>{{ $t('messages.note_msg') }}. <a href="https://fontawesome.com/search" target="_blank">{{ $t('messages.click_here') }}
              ...
              <div class="form-group" v-if="is_route && isLinkMenuType">
                <label for="menu-route" class="form-label">{{ $t('messages.lbl_route_list') }} <span class="text-danger">*</span></label>
...
const validationSchema = yup.object({
  title: yup.string().required(t('messages.title_required')),
  menu_item_type: yup.string().required(t('messages.menu_item_type_required')),
  target_type: yup.string().required(t('messages.target_type_required')),
  url: yup.string().test('required', t('messages.url_required'), function (value) {
    if (menu_item_type.value == 'link' && is_route.value === 0 && !value) {
      return false;
    }
    return true
  }).url(),
  route: yup.string().test('required', t('messages.route_required'), function (value) {
