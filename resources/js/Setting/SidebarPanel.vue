      ...
      <div class="card">
        <div class="card-body">
          <div class="list-group list-group-flush" id="setting-list">
            ...
            <div class="mb-3" :class="hasPermissions('setting_currency') ? 'active-menu' : ''" id="Settings.currency-settings" v-if="hasPermissions('setting_currency')" @click="toggle()"><router-link :to="{name: 'Settings.currency-settings'}" class="btn btn-border"><i class="fa fa-dollar fa-lg me-2"></i>{{ $t('setting_sidebar.lbl_currency_setting') }} </router-link></div>
            <div class="mb-3" :class="hasPermissions('setting_commission') ? 'active-menu' : ''" id="Settings.commission" v-if="hasPermissions('setting_commission')" @click="toggle()"><router-link :to="{name: 'Settings.commission'}" class="btn btn-border"><i class="fa fa-percent fa-lg me-2"></i>{{ $t('setting_sidebar.lbl_commission') }}</router-link></div>
            <div class="mb-3" :class="hasPermissions('setting_holiday') ? 'active-menu' : ''" id="Settings.holiday" v-if="hasPermissions('setting_holiday')" @click="toggle()"><router-link :to="{name: 'Settings.holiday'}" class="btn btn-border"><i class="fa-solid fa-calendar-check me-2"></i>{{ $t('setting_sidebar.lbl_holiday') }}</router-link></div>
